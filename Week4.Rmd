...
hh_data1 <- read_csv("households_ma.csv") %>%
   mutate(num_units = case_when(
    ...
    BLD_label=="10-19 Apartments"~"10 to 49 apartments",
    ...
    BLD_label=="20-49 Apartments" ~ "10 to 49 apartments",
  ...
  mutate(hh_type = case_when(
    HHT_label =="Married couple household"~"1Married couple",
    ...
    HHT_label =="Nonfamily household: Female householder: Living alone"~"Living Alone",
    HHT_label =="Nonfamily household: Male householder: Living alone"~"Living Alone"))
```

```{r}
model <- lm(gross_rent ~ age_struct + bedrooms + hh_type + num_units, 
            data = hh_data1)
