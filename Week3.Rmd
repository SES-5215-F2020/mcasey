...
#### Recode
##### Changed my household type thanks to Ryan's code, didn't think to make those more simple before! I decided to do this later in time frame, and for some reason I couldn't get both mutates to work for the data, so I kept them separate (to be fixed at a later date!).
```{r message = FALSE}
...
hh_data2 <- read_csv("households_ma.csv") %>%
...
Age of Building and Number of Bedrooms: weak (0.11 correlation), but statistically significant (p-value < 2.2e-16), positive correlation at a 95% confidence level.
>>>>>>> 5ceae9f678f39827320cb08e322d097deea32834
...
```{r}
chi_sq <- chisq.test(hh_data2$hh_type, hh_data1$num_units)
chi_sq
...
```{r}
anova1 <- aov(age_struct ~ hh_type, data = hh_data2)
summary(anova1)
...
ggplot(hh_data2, aes(x=hh_type , y = age_struct)) +
...
```{r}
anova3 <- aov(bedrooms ~ hh_type, data = hh_data2)
summary(anova3)
...
ggplot(hh_data2, aes(x=bedrooms , y = hh_type)) +
...
```{r}
anova5 <- aov(num_persons ~ hh_type, data = hh_data2)
summary(anova5)
...
ggplot(hh_data2, aes(x=num_persons , y = hh_type)) +
...
```{r}
anova7 <- aov(move_in ~ hh_type, data = hh_data2)
summary(anova7)
...
ggplot(hh_data2, aes(x=move_in , y = hh_type)) +
...
```{r}
anova9 <- aov(gross_rent ~ hh_type, data = hh_data2)
summary(anova7)
...
ggplot(hh_data2, aes(x=gross_rent , y = hh_type)) +
...
#### 8. Gross Rent and Number of Units
```{r}
